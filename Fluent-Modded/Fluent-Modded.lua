-- This shit is a modification of Fluent-Dawid from Vraigos/FluentModed, I modified it to be more readable and to make it look great to look at this  lua
...
function Animation.Apply(Theme, Root)
	...
	if not Theme or not Root or not Library.Animated or not Theme.ShineEnabled or not Theme.Shine then
		return
	end
	...
	for _, Object in ipairs(Root:GetDescendants()) do
		if Object:IsA("UIGradient") then
			...
			Connection = RunService.RenderStepped:Connect(function(DeltaTime)
				local Time = Object:GetAttribute("old_t") or 0
				Time += DeltaTime * Speed
				Object:SetAttribute("old_t", Time)

				Object.Rotation = (Time * RotationSpeed) % 360
				Object.Color = ColorSequence
			end)
			table.insert(AnimationConnections, Connection)
		end

		if Object:IsA("UIStroke") and Theme.StrokeShine then
			local FromColor = Theme.StrokeDark or Theme.AcrylicBorder
			local ShineColor = Theme.Accent
			Connection = RunService.RenderStepped:Connect(function(DeltaTime)
				local Time = Object:GetAttribute("old_t") or 0
				Time += DeltaTime * Speed
				Object.Thickness = 2
				...
				Object.Color = FromColor:Lerp(ShineColor, (math.sin(Time) + 1) / 2)
			end)
			table.insert(AnimationConnections, Connection)
		end
	end
...
function Creator.UpdateTheme()
	...
	local CurrentTheme = Themes[Library.Theme]
	if Library.Window and Library.Window.AcrylicPaint then
		Animation.Apply(CurrentTheme, Library.Window.AcrylicPaint.Frame)
		getgenv().ButtonGradients = CurrentTheme.ButtonGradient
	end
end

function Library:ToggleAnimated(Value)
	Library.Animated = Value
	Creator.UpdateTheme()
end
...
ElementsTable.Colorpicker = (function()
	...
	function Element:New(Idx, Config)
		...
		local function CreateColorDialog()
			...
			Dialog.Root.Size = UDim2.fromOffset(430, 370)
			...
			local OldColorFrameChecker = New("ImageLabel", {
				...
				TileSize = UDim2.fromOffset(40, 40),
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(114, 255),
			...
			local DialogDisplayFrameChecker = New("ImageLabel", {
				...
				TileSize = UDim2.fromOffset(40, 40),
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(20, 255),
				Size = UDim2.fromOffset(86, 24),
				Parent = Dialog.Root,
			}, {
				New("UICorner", {
...
ElementsTable.Slider = (function()
	...
	function Element:New(Idx, Config)
		...
		Creator.AddSignal(SliderDot.InputBegan, BeginDrag)
		Creator.AddSignal(SliderDot.InputEnded, EndDrag)
		Creator.AddSignal(SliderHitbox.InputBegan, BeginDrag)
		Creator.AddSignal(SliderHitbox.InputEnded, EndDrag)
...
local InterfaceManager = {} do
		...
		if isfile(Path) then
			...
			if Success then
				for Key, Value in next, Decoded do
					InterfaceManager.Settings[Key] = Value
				end
			end
		end
	end
	...
	function InterfaceManager:BuildInterfaceSection(Tab)
		...
		Lib:ToggleAnimated(Settings.Animated)
	end
end
...
local function DeserializeUDim2(Value)
	return UDim2.new(Value.ScaleX, Value.OffsetX, Value.ScaleY, Value.OffsetY)
end
...
function FloatingButtonManager:SetLibrary(Lib)
	self.Library = Lib
end
	...
	for Id, Saved in pairs(Decoded) do
		...
		if Frame then
			...
			if Saved.isCircle then
				...
				if Button then
					local Shortest = math.min(Frame.AbsoluteSize.X, Frame.AbsoluteSize.Y)
					Button.TextScaled = true
					Button.TextSize = math.floor(Shortest * 0.45)
				end
			else
				if Corner then
...
return Library, SaveManager, InterfaceManager, FloatingButtonManager
